...
# image save location
IMG = "./img/"
DATA = "./datasets_output/"

norm_data = pd.read_csv(DATA+"normalized_data.csv")
percent_df=pd.read_csv(DATA+'tract_percents.csv')
population_df = pd.read_csv(DATA+"population_size.csv")

# Load GEOjson

#Allegheny County
gdf = gpd.read_file("mapdata/Allegheny_County_Census_Tracts_2020_2458367900982142177.geojson")
#Allegheny County Districts
...
fig.savefig(IMG+'cloropleth.png', dpi=300)
fig.clear()



## Cloropleth with overlay
fig, ax = plt.subplots(1, 1, figsize=(15, 10))
merged_gdf.plot(column='risk_score', ax=ax, legend=True,
           cmap='OrRd',  # Colormap (e.g., 'Viridis', 'Blues', 'OrRd')
           legend_kwds={'label': "Risk Score",
...
fig.savefig(IMG+'cloropleth2.png', dpi=300)
fig.clear()
...
allegheny.apply(lambda x: ax.annotate(text=x['LABEL'], xy=x.geometry.centroid.coords[0], ha='center'), axis=1)
rivers.plot(ax=ax, facecolor='blue', edgecolor='blue', linewidth=4)
fig.savefig(IMG+'cloropleth4.png', dpi=300)
fig.clear()
...
rivers.plot(ax=ax, facecolor='blue', edgecolor='blue', linewidth=4)
allegheny.apply(lambda x: ax.annotate(text=x['LABEL'], xy=x.geometry.centroid.coords[0], ha='center'), axis=1)
fig.savefig(IMG+'cloropleth5.png', dpi=300)
fig.clear()
...
# allegheny.apply(lambda x: ax.annotate(text=x['LABEL'], xy=x.geometry.centroid.coords[0], ha='center'), axis=1)
fig.savefig(IMG+'cloropleth6.png', dpi=300)
fig.clear()
...
fig.savefig(IMG+'population_boxplot.png', dpi=300)
fig.clear()
...
# fig.set_tight_layout(True)
fig.savefig(IMG+'corr_heatmap.png', dpi=300)
fig.clear()
